...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetchStoreApi(`${WP_URL}/wp-json/custom/v1/login`, {
      method: "POST",
    ...
    return NextResponse.json({ success: false, code: "backend_unavailable", message: "Login is temporarily unavailable. Please try again shortly." }, { status: 503 });
  }
}
