...
export async function getHomepageImages() {
  ...
  try {
    return await fetchJson<any>(`${WP_URL}/wp-json/custom/v1/homepage-images`, CATALOG_CACHE_SECONDS, [CACHE_TAGS.catalog]);
  } catch {
    return null;
  }
}
...
export async function getProduct(slug: string) {
  const products = await request<any[]>(`/products?slug=${encodeURIComponent(slug)}`);
